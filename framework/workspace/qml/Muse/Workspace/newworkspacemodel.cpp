...
void NewWorkspaceModel::validateWorkspaceName()
{
    ...
    if (m_workspaceNames.contains(m_workspaceName)) {
        m_errorMessage = muse::qtrc("workspace",
                                    "A workspace with the name “%1” already exists. Please choose a different name.")
                         .arg(m_workspaceName);
        return;
    }
    ...
    if (!muse::io::isAllowedFileName(muse::io::path_t(m_workspaceName))) {
        m_errorMessage = muse::qtrc("workspace",
                                    "“%1” cannot be used as a workspace name. Please choose a different name.")
                         .arg(m_workspaceName);
        return;
    }
}
