...
TEST_F(Ui_QmlDataFormatterTests, FormatReal_TrimsLocalizedZeroDigitAndSeparator)
{
    // Arabic uses Eastern Arabic-Indic digits and a decimal separator that
    // are neither ASCII '0' nor '.'; trimming must recognize the locale's
    // own tokens, not hard-coded ASCII ones.
    const QLocale arabic("ar");
    QLocale::setDefault(arabic);
    ASSERT_NE(arabic.zeroDigit(), QStringLiteral("0"));
    ASSERT_NE(arabic.decimalPoint(), QStringLiteral("."));

    const QString half = m_formatter.formatReal(1234.5, 2);
    EXPECT_FALSE(half.endsWith(arabic.zeroDigit()));
    EXPECT_TRUE(half.contains(arabic.decimalPoint()));

    const QString whole = m_formatter.formatReal(1234.0, 2);
    EXPECT_FALSE(whole.contains(arabic.decimalPoint()));
}
...
TEST_F(Ui_QmlDataFormatterTests, DecimalsForStep)
{
    EXPECT_EQ(m_formatter.decimalsForStep(1.0), 0);
    EXPECT_EQ(m_formatter.decimalsForStep(0.5), 1);
    EXPECT_EQ(m_formatter.decimalsForStep(0.01), 2);
    EXPECT_EQ(m_formatter.decimalsForStep(0.0001), 4);
    EXPECT_EQ(m_formatter.decimalsForStep(-0.25), 2);
    EXPECT_EQ(m_formatter.decimalsForStep(1e-6), 6); // capped
}
